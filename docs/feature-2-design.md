...
> **Extracted (Issue #30, Phase 0 完結 sweep)**: 本 section は sub-PR 構造を持つ任意の chain merge に適用される一般メタ運用のため、 [`docs/chain-merge-pattern.md`](chain-merge-pattern.md) に extract した。 SoT は新 doc を参照。 Feature 2 が本 pattern の origin (= 確立 case: PR #27 review M-3 / Issue #28、 checklist の canonical 育成リストは Feature 2 contract symbol が基底)。

直近適用先: sub-PR 3 (#20、 retry) 着手前 (詳細・checklist 本体・PR description form は extract 先を参照)。
