...
# admission-gate 実行 (original_stop == HUMAN のとき label / grey-zone を判定し turn.gate_result を populate、§3.5)
...
**実行順序が D22 に従うこと**: `rule_stop_reason` → §3.3.a → admission-gate → §3.3.b の順で並ぶ。両フックの間に admission-gate が挟まる (D22) のは、§3.3.b が読む `turn.gate_result` を admission-gate が populate するため。逆順 (admission-gate → §3.3.a → §3.3.b) にすると §3.3.a active-mode Track B が `stop` を `HUMAN` に mutate した turn では admission-gate が既に走り終わっているため、その新規 HUMAN escalation は admission-gate を経由せず §3.3.b の grey-zone gating も skip される（`original_stop is None` ∴ §3.3.b の entry-guard で早期 return する）。この順序制約は snippet の comment 順序をそのまま踏襲すれば守れる。
...
- **`turn`**: read-only の domain object arg。Decider は turn を書き換えない (書き換えるのは Conductor loop 側の `turn.gate_result` populate のみ、§3.5 admission-gate 責務)。
