...
**shield が破れる条件（=昇格 trigger）**: (a) matrix job で concurrency limit が効き、一部の job が CheckRun として suite に足された状態で queued に留まる（本 repo は matrix なし）、(b) `needs:` dependency chain で待たされた downstream job が同様に queued CheckRun として rollup に載る（本 repo は依存なし）、(c) GitHub Actions API の platform-level 変更で queued 段階の CheckRun が公開される。(a)-(b) は workflow shape の追加/変更で生じ得るため、`.github/workflows/*.yml` に matrix・`needs:` が入る PR は本行の shield 前提を壊し得る（そういう PR が入ったら本行を昇格に回すこと）。**逆に、`StatusContext`（e.g., 外部 CI via commit statuses）は rollup に混ざっても shield を破らない** — `createdAt` が non-null 保証なので observed に必ず貢献する（むしろ observed の下限を suite 作成前まで押し下げ、`head_committed_date` の 12h 早鳴りを積極的に防ぐ方向に働く）。以前の版が (a) として `StatusContext` を挙げていたのは事実誤認 — msg-(gate) v0.3.4 round-3 correctness で訂正、shield-breaking 列挙から除去した。
