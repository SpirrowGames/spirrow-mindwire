...
**shield が破れる条件（=昇格 trigger、`RES-WIRING` landing 後のみ意味を持つ）**: (a) `RES-WIRING` PR が caller-side defense を実装せずに配線を投入する（obligation 違反、PR review で catch すべき）、(b) 実装しても matrix job で concurrency limit が効いて queued CheckRun が rollup に残る、(c) `needs:` dependency chain で待たされた downstream job が同様に queued CheckRun として rollup に載る、(d) GitHub Actions API の platform-level 変更で defense を跨ぐ CheckRun shape が出現する。(a) は landing 時 review で防ぐ; (b)-(d) は workflow shape の追加/変更で生じ得るため、`.github/workflows/*.yml` に matrix・`needs:` が入る PR は本行の shield 前提を壊し得る（そういう PR が入ったら本行を昇格に回すこと）。**逆に、`StatusContext`（e.g., 外部 CI via commit statuses）は rollup に混ざっても shield を破らない** — `createdAt` が non-null 保証なので observed に必ず貢献する（むしろ observed の下限を suite 作成前まで押し下げ、`head_committed_date` の 12h 早鳴りを積極的に防ぐ方向に働く）。以前の版が shield-breaking (a) として `StatusContext` を挙げていたのは事実誤認（msg-(gate) v0.3.4 round-3 correctness で除去済）。
...
**PR-review msg-(gate) v0.3.4 round-2/round-4 の shield rewording の扱い**: round-2 で naysayer は「Actions は push 直後に CheckRun を queued 状態で作る、∴ rollup ≠ ∅ ∧ observed = ∅ が queue 窓で成立する」と主張した。私は当時「SpirrowGames の実 workflow shape では成立しない」と empirical に反論し 4 点構成に書き直したが、round-4 で naysayer は「rest 状態の観測は queue 窓を通過した後であり sampling が不適切」と methodological な指摘を返し、これは正しい。∴ round-4 で「empirical shield」主張を撤回し、deferral の根拠を **(A) `RES-WIRING` により `gate_admission` は呼び出し元数=0（unreachable-in-production）** と **(B) `RES-WIRING` landing 後は caller obligation が唯一の shield** の 2 層に立て直した。round-2 fix が入れた caller obligation の項は正しい判断で、round-4 版で B として structural に位置付け直した。本行の deferred 判断は据え置き（unreachable-in-production は今日成立、caller obligation は landing 時に catch すべき review 対象）。
