...
**invariant scope の明示**（`SpirrowGames/spirrow-mindwire#290` pr-gate review 対応、`8d9549f` に対する REQUEST_CHANGES verdict）: D-4 が保護する invariant「material-side outage を notification-side outage に転化しない」は **runtime における material service の到達不能**（magickit service の一時的 down、network partition、tailnet unreachable 等の runtime resilience 領域）を対象とし、**deploy-time における config 未設定**（`MINDWIRE_DECISION_DASHBOARD_URL` の unset）は対象**外**。deploy 時 config error は「deploy が完了していない状態」であって runtime における部分的な degradation ではない ∴ D-1 の init throw（deploy 完了検証の loud fail）と D-4 の invariant（runtime outage の fail-open）は subject が異なる。両者は両立する — env が set 済で material service が runtime に down した場合は従来通り fail-open で吸収し、env が unset の deploy は init throw で loud に検出して deploy 完了前に修正させる。この境界を曖昧にすると (i) config error の silent 伝播で誤 URL の human-facing link 送出（D-3 の副次的害）を許すか、(ii) runtime outage の全体停止で通知系まで巻き添えにする — いずれも設計意図と反する。
