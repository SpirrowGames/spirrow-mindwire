...
- **naysayer** — independent, adversarial review under 5 principles (YAGNI/OverScope / hybrid & dual-management complexity / no opposition for opposition's sake / explicitly endorse what should be endorsed / silence is negligence). The canonical, versioned definition is [`spec/NAYSAYER_PRINCIPLES.md`](spec/NAYSAYER_PRINCIPLES.md) (this list is a summary; the spec is the SOT — ADR-2026-06-03-17). Beyond the PR-gate, the naysayer also participates at **design time**: `scripts/design_review.py` relays its critique into a design thread.
