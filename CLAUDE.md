...
ADR 本体の所在は移行中: Drive 未反映だったものは本リポジトリ `docs/adr/` に、Drive 反映済みのものは引き続き Drive にある (内訳は `docs/adr/README.md`)。本 CLAUDE.md からは参照のみで、ADR の規範定義を本ファイルに転載しないこと (F-04 / F-07 系の分散症状を回避)。
