...
# --------------------------------------------------------------------------- #
# The mask must scan SEQUENTIALLY.
#
# A heredoc body is inert text, so anything inside one that looks like an opener
# is not an opener. The first version (#154) scanned with `finditer` over the
# whole string and missed that, which the Tier B naysayer turned into a working
# bypass of the entire classifier. These pin the exploit and its mirror image.
# --------------------------------------------------------------------------- #


def test_fake_opener_inside_a_body_cannot_blank_the_command_after_it() -> None:
    # The naysayer's payload on PR #154, verbatim. bash ends the heredoc at EOF
    # and then runs `rm -rf /`. The old scan read the literal `<<'echo'` on line 2
    # as a real opener, took `; gh pr create` as its owner, found a terminator
    # line `echo` at the very end, and blanked everything between — `rm -rf /`
    # included — from BOTH the structural pass and the coarse floor.
    cmd = "git commit -F - <<'EOF'\n; gh pr create <<'echo'\nEOF\nrm -rf /\necho"
    assert classify_tool_call("Bash", {"command": cmd}).operation is Operation.FS_DELETE


def test_fake_sink_opener_inside_a_non_sink_body_is_not_an_opener() -> None:
    # The mirror image: the outer heredoc is NOT masked (bash is not a data sink),
    # so its body is script. A sink-looking opener inside it must still not be
    # treated as one, or the same hole reappears with the roles reversed.
    cmd = "bash <<'EOF'\n; git commit -F - <<'X'\nEOF\nrm -rf /tmp/a\nX"
    assert classify_tool_call("Bash", {"command": cmd}).operation is Operation.FS_DELETE


def test_unterminated_body_stops_the_scan_rather_than_guessing() -> None:
    # Without a terminator we cannot tell where the body ends, so we cannot skip
    # it either. Everything from there stays unmasked and is read as shell.
    cmd = "git commit -F - <<'A'\nprose\nA\ngit commit -F - <<'B'\nrm -rf /tmp/x"
    assert classify_tool_call("Bash", {"command": cmd}).operation is Operation.FS_DELETE


def test_two_sequential_sinks_are_both_masked() -> None:
    # The legitimate multi-heredoc case still works: commit, then open the PR.
    cmd = (
        "git commit -F - <<'A'\nmentions `rm` here\nA\n"
        "gh pr create --body-file - <<'B'\nmentions `git rm` here\nB"
    )
    assert classify_tool_call("Bash", {"command": cmd}).operation is Operation.GIT_COMMIT


def test_a_deletion_between_two_sinks_is_still_seen() -> None:
    cmd = (
        "git commit -F - <<'A'\nprose\nA\nrm -rf /tmp/x\ngh pr create --body-file - <<'B'\nprose\nB"
    )
    assert classify_tool_call("Bash", {"command": cmd}).operation is Operation.FS_DELETE
...
def test_a_body_cannot_supply_the_next_heredocs_owner_name() -> None:
    # The owner is read from the text between the previous terminator and this
    # opener, not from the whole prefix — otherwise an earlier body could name a
    # sink and license masking a body that is not one.
    cmd = "bash <<'A'\ngit commit -F -\nA\ntee out <<'B'\nrm -rf /tmp/x\nB"
    assert classify_tool_call("Bash", {"command": cmd}).operation is Operation.FS_DELETE
