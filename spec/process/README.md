...
`moved_from` は Python literal (`path::LITERAL_NAME`) と doc section (`path::§HEADING`) の両方を受ける opaque string。**現時点で origin を持つのは Python literal 例の `OBL-DECLARE-UNREADABLE` / `OBL-VERDICT-CONSTRAINT` の 2 件のみ**。`OBL-READBACK-ENTRY` / `OBL-READBACK-EXIT` は旧 `CLAUDE.md §N.1.3 / §N.1.4` の spirit を英文で再定式化したもの (Japanese の numbered list 構造は verbatim には prompt body へ落ちない) で、**verbatim 移設ではなく net-new formulation** ゆえ `origin` を持たない — canary 二重プライムの length 不変式は verbatim 移設の text にのみ適用される (前身が Japanese の要件リストで、英文 body の length と一致させても不変式の意味を持たない)。PR #135 Tier-B naysayer Finding 1 が過去の origin 記載の誤りを指摘し、本 PR で訂正した。
...
9. **base-read 判別 (msg-766 Tier-B Finding 3)** — 「空 base」= advisory の緑を返す挙動は、`git show` が **「その revision に file 実体が無い」stderr** を出した場合 (added-in-this-PR-manifest ケース) に **限定**する。git binary 欠損 / unfetched ref / ambiguous argument / network 系失敗など**それ以外の失敗**は tool crash として `ObligationsReadbackToolError` を投げ、msg-762 契約に従って check を赤にする。前身は "any non-zero returncode → empty base" と swallow していたため、broken checkout も含めて緑と誤報告する path があった (Finding 3 の原文: "the previous swallow recreated the exact fail-open the contract was designed to prevent")。判別は `_GIT_FILE_MISSING_STDERR_MARKERS` (`"does not exist in"` / `"exists on disk, but not in"`) の substring 一致で行う。4 本の契約テスト (`test_base_read_treats_file_missing_at_revision_as_empty_base` / `test_base_read_treats_exists_on_disk_stderr_as_empty_base` / `test_base_read_unknown_ref_raises_tool_error` / `test_base_read_missing_git_binary_propagates_oserror`) がこの規則を対で守る。
10. **git ロケール固定 (msg-768 Tier-B Finding 1)** — #9 の substring matcher は英語文字列を対象とするため、`_read_base_manifest` は `subprocess.run` に `env={..., "LC_ALL": "C", "LANG": "C"}` を明示的に渡す。ambient shell が `ja_JP.UTF-8` 等の場合、git が localised stderr (`fatal: パス '...' は '...' に存在しません`) を吐くと substring は miss し、正当な empty base 結果を tool crash と誤報告する。`OBL-PRCHECK-READ` を localhost で実行する implementer が影響を受ける経路 = 本 obligation が明示的に「ローカルで base に checkout し同一スクリプトを再実行して確認」を求めるため、host locale に依存させると本 obligation 自身が壊れる。契約テスト = `test_base_read_forces_c_locale_for_git`。
11. **reference-scan の Unicode 耐性 (msg-768 Tier-B Finding 2)** — `_scan_references` は scanned file を `read_text(encoding="utf-8", errors="replace")` で読む。前身は `UnicodeDecodeError` を `try/except` で捕捉して `continue` していたため、単一の非 UTF-8 byte (例: ISO-8859-1 の smart quote / é 等が混ざった .md doc) がある file の中の全 `OBL-*` 参照を silent に脱落させ、advisory が緑で lying report を返しうる (§「fail-open の宣言先を先に決める」直接違反)。regex が探すのは ASCII の `OBL-*` token のみ ∴ 非 ASCII 領域を U+FFFD で置換するのは reference detection の accuracy に影響しない (skip する方が strictly worse)。契約テスト = `test_reference_scan_survives_non_utf8_bytes_in_a_scanned_file`。
12. **rename heuristic の origin-drop 耐性 (msg-768 Tier-B Finding 3)** — `_match_rename` の body-match fallback は old 側の `moved_from` 状態を gate 条件にしない。前身は `old.moved_from is None AND candidate.moved_from is None` を要求していたため、「同一 commit で origin 分類を drop しつつ rename する」ケース (本 PR の `OBL-READBACK-ENTRY/EXIT` の origin 削除は rename 抜きだが、同型 pattern は今後も起こる) が両分岐を素通りし削除 + 追加として誤報告されうる。契約テスト = `test_body_match_rename_survives_dropping_origin_block`。
