...
| # | 内容 | 処分（2026-09-10） |
|---|---|---|
| **残余-1** | **`_MAX_DIFF_CHARS`（Baseline `A/D-1`）の値そのものの再測定。** | **[`#273`](https://github.com/SpirrowGames/spirrow-mindwire/issues/273) に container 化（Tier-C msg-3027 order B、Bohr msg-3035/msg-3037 の edit set E-8）。** 本残余は 2 つの独立した precondition のもとにあった: (a) post-elision 分布（本設計が着地した後の）、および (b) `T-gate-reads-stale-base-diff` が変える denominator（`spec/design/T-gate-reads-stale-base-diff.md:157` 参照、#11 が `202,849 → 118,107` を明示）。abort により **(a) は void**（elision は来ない ∴ raw = post-elision — dead precondition として記録）。**(b) は生きている**（stale-base 修正が denominator を per-PR-variable な量だけ縮める） ∴ 再測定は「stale-base 修正の着地後、その修正が変えた denominator に対して」行う。**BLOCKED-UNTIL: `SpirrowGames/spirrow-mindwire` の `main` に stale-base 修正が着地。着地するまで `_MAX_DIFF_CHARS` の新値を提案してはならず、`#273` を close してはならない。**（`T-gate-reads-stale-base-diff.md` を releasing event の checkable 名前として使う — 「着地したか」は思い出すのではなく調べる。）**No rescaling**: pre-fix 数値を arithmetic に post-fix 推定へ換算する経路は禁止（#11 の 118,107/202,849 = 0.582 は 1 標本であり scalar correction ではない、msg-3037 §3）。**Cancellation clause**: stale-base lane 自体が撤回/中止された場合、precondition は共に死に、`#273` は then-current regime を明示したうえで unblock する（msg-3037 §4 item 2）。**Filing 時点の観測**: releasing event は既に発火済み — PR [#211](https://github.com/SpirrowGames/spirrow-mindwire/pull/211) が `f6dfe441` として 2026-08-31 に `main` へ merge された ∴ `#273` は「released」状態で生まれる。BLOCKED-UNTIL 節は record として保持する（releasing event の identity を checkable に留めるため）。**Door 補正（規律-18 audible debt）**: stale-base lane の tracking PR は既に merged で open な surface が存在しない ∴ door を刺す場所が無い。`T-gate-reads-stale-base-diff.md` は他 lane の live SOT であり本 PR は編集しない（msg-3037 §3）— pointer は本節に audible debt として置く。**gate の advisory への処分**: msg-2961（`#255` PR-gate の structure advisory）が objected した「stale-base 側で pick up される見込み（そちらの残余ではないが、着地の副作用として自然に問題領域に入る）」の文は **削除された**（softened ではなく deleted、msg-3035 §4）。 |
...
6. **残余の terminal state は §5 に記述**: 残余-1 は [`#273`](https://github.com/SpirrowGames/spirrow-mindwire/issues/273) に container 化し **BLOCKED-UNTIL: stale-base 修正が `main` へ着地** で hard-block（no rescaling、cancellation clause、filing 時点で releasing event PR #211 = `f6dfe441` が既に観測されている旨を record — Tier-C msg-3027 order B に基づき Bohr msg-3035/msg-3037 が確定、Einstein msg-3036 の blocking objection と msg-3038 の advisory を受けた最終形）、残余-2/3/4/5 は [`#254`](https://github.com/SpirrowGames/spirrow-mindwire/issues/254) に container 移動して door を空のまま audible debt として残す、残余-7 と 補償(a) は MOOT。`#231` は Phase B abort を close reason にして close する（re-home しない）。
7. **本 abort PR の close 条件 (Tier-C msg-3027 order B 由来)**: 上記 clause 6 の 残余-1 分の container（`#273`）が実在し、本 §5 の 残余-1 行から pointer が張られていること。これは msg-2961 の PR-gate structure advisory への処分であり、gate 再発火は new head に対して ordinary loop operation として行われる（msg-3035 §5）。**merge は依然 Tier-C（Takahito）**; loop は merge しない。
...
| 17 | **記録すべき item の routing 先として、まだ存在しない container（実装スレッド）を名指した。** container 名がある ∴ 保存に見える。しかし routing の瞬間にその thread は addressable ではなく、enumeration は不可能 ∴ この routing は drop と区別できない。**該当箇所は msg-2589 の "so it is addressed in the implementation thread rather than in this one" という routing 部分**（msg-2589 の別の要求「open a separate tracked follow-up item」自体は tracking container を求めており、この error に該当しない — 「未存在の container を宛先にした」routing 部分だけがこれに該当する）。**msg-2619 §0 が routing 部分の container 補正（未存在の実装スレッド → 今 addressable な durable container = GitHub issue）を挿入し、§4 で規律-17 として閉じた**。「addressed in the implementation thread」の意図は container 補正後も preserved: 実装スレッドの `OBL-READBACK-ENTRY` が pointer を pick up する。本 error type は「見ていないものを黙って落とす」本スレッド founding pathology の container 版 — silence でなく **misdirection** を経由する変異 | msg-2619 §4 | **規律-17** |
...
| msg-2589 (human C) | 逐語: "Merge PR #208 into main now, and at the same time open a separate tracked follow-up item recording the basename-only matching gap so it is addressed in the implementation thread rather than in this one." — 二半分: (a) merge PR #208、(b) 追跡 follow-up item を起票し、実装スレッドで対処 | **partially reflected**（(a) merge は Takahito が 2026-08-31T04:59:14Z に実施、merge commit `b4eef73`。(b) の container は msg-2619 §0 が「実装スレッド（未存在）」→「durable な GitHub issue（今 addressable）」に補正した — msg-2589 の「addressed in the implementation thread」意図は container 補正後も preserved: 実装スレッドの `OBL-READBACK-ENTRY` が 残余-7 pointer → issue #231 を pick up する。item 起票と 残余 pointer 追加は本 followup commit。§付録 G error type 17 が routing 部分と tracking 要求部分を区別しつつ container 補正の該当箇所を pin する） | GitHub merge、[`#231`](https://github.com/SpirrowGames/spirrow-mindwire/issues/231)、§5 残余-7、§付録 G error type 17 |
