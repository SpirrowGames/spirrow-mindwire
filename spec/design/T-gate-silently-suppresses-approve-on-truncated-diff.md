...
| # | 内容 | 処分（2026-09-10） |
|---|---|---|
| **残余-1** | **`_MAX_DIFF_CHARS`（Baseline `A/D-1`）の値そのものの再測定。** | **snapshot（規律-11 の適用）**: 本セルは起票時点のスナップショットであり、live rule の SOT は [`#273`](https://github.com/SpirrowGames/spirrow-mindwire/issues/273) の body（以後の変更は issue 側で行われ、本 terminal record は追随しない）。 **[`#273`](https://github.com/SpirrowGames/spirrow-mindwire/issues/273) に container 化（Tier-C msg-3027 order B、Bohr msg-3035/msg-3037 の edit set E-8、Bohr msg-3059 の edit set E-10/E-13 で snapshot 化）。** 本残余は 2 つの独立した precondition のもとにあった: (a) post-elision 分布（本設計が着地した後の）、および (b) `T-gate-reads-stale-base-diff` が変える denominator（`spec/design/T-gate-reads-stale-base-diff.md:157` 参照、#11 が `202,849 → 118,107` を明示）。abort により **(a) は void**（elision は来ない ∴ raw = post-elision — dead precondition として記録）。**(b) は生きている**（stale-base 修正が denominator を per-PR-variable な量だけ縮める） ∴ 再測定は「stale-base 修正の着地後、その修正が変えた denominator に対して」行う。**BLOCKED-UNTIL（discharged）**: releasing event は起票時点で既に発火済み — PR [#211](https://github.com/SpirrowGames/spirrow-mindwire/pull/211) が `f6dfe441` として 2026-08-31 に `main` へ merge された ∴ `#273` は「released」状態で生まれ、本 record 上の block はもはや live block ではなく履行済みの前提条件として保持される。record 上に残す理由は releasing event の identity（`#211` = `f6dfe441`）を checkable に留めること、および `#273` に提案される任意の値が pre-fix regime に基づいてはならないという regime discipline の起点として使うため（msg-3059 §1、advisory 1 disposition: cancellation clause 削除 + BLOCKED-UNTIL は discharged として保持）。**No rescaling**: pre-fix 数値を arithmetic に post-fix 推定へ換算する経路は禁止（#11 の 118,107/202,849 = 0.582 は 1 標本であり scalar correction ではない、msg-3037 §3）。**Door 補正（規律-18 audible debt）**: stale-base lane の tracking PR は既に merged で open な surface が存在しない ∴ door を刺す場所が無い。`T-gate-reads-stale-base-diff.md` は他 lane の live SOT であり本 PR は編集しない（msg-3037 §3）— pointer は本節に audible debt として置く。**gate の advisory への処分**: msg-2961（`#255` PR-gate の structure advisory）が objected した「stale-base 側で pick up される見込み（そちらの残余ではないが、着地の副作用として自然に問題領域に入る）」の文は **削除された**（softened ではなく deleted、msg-3035 §4）。 |
...
6. **残余の terminal state は §5 に記述**: 残余-1 は [`#273`](https://github.com/SpirrowGames/spirrow-mindwire/issues/273) に container 化（rule の live SOT は `#273`、characterisation と regime note は §5 の 残余-1 行 — Tier-C msg-3027 order B に基づき Bohr msg-3035/msg-3037/msg-3059 が確定）、残余-2/3/4/5 は [`#254`](https://github.com/SpirrowGames/spirrow-mindwire/issues/254) に container 移動して door を空のまま audible debt として残す、残余-7 と 補償(a) は MOOT。`#231` は Phase B abort を close reason にして close する（re-home しない）。
7. **本 abort PR の close 条件 (Tier-C msg-3027 order B 由来)**: 上記 clause 6 の 残余-1 分の container（`#273`）が実在し、§5 の 残余-1 行から pointer が張られていること。これは msg-2961 の PR-gate structure advisory への処分であり、gate 再発火は new head に対して ordinary loop operation として行われる（msg-3035 §5）。**merge は依然 Tier-C（Takahito）**; loop は merge しない。
