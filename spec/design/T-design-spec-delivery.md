...
- **D-25′（改訂の運び方 — base-hash ＋ 構造アンカー ＋ NEW のみ）** proposer の改訂は対象ファイルの blob sha（`base`）を名指す。implementer は編集前に照合し、一致しなければ `BASE_MISMATCH` で停止する。これが陳腐化検査であり、逐語 byte を 1 文字も要さない。
  ...
  - **同じ形は、本書が統べる review — proposer の spec-review — に適用する。** implementer が適用した出荷物の照合は「対象ブロックを逐語引用せよ」ではなく「読んだ blob sha ＋ 位置（§／id／行番号）を述べよ」で行う。proposer にも打ち直しをさせない。
  - **独立 naysayer の PR-gate は本書の外にある。** gate の義務は本書ではなく gate 自身の system prompt が定める ∴ **本書はそれを縛らない** — 決定・義務文・受け入れ条件のいずれも、gate に「何を引用するな」「何を書け」と指示しない。**本書が発行する obligation は `role: implementer` の 3 件のみであり（§4-1 / §4-2 / §4-3）、`role: naysayer` の entry を発行しない** ∴ 本書の payload が gate の prompt に届く経路そのものが無い。gate の要求を本書に書き写すこともしない — 写せば正本が 2 つになり、D-31 が A-29 で払った代償を、照合機構を持たないまま抱えることになる。
